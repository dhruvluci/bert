...
def read_squad_data(input_file):
...
def get_qa(path):
	...
	model_request.inputs['examples'].CopyFrom(tf.contrib.util.make_tensor_proto(c, dtype=tf.string, shape=[batch_size]))
...
def get_qa2(stringx):
	def process_inputs(input_data):
		...
		eval_writer.close()
		return eval_examples, eval_features
	features=process_inputs(stringx)
	hostport="34.74.195.118:8500"
	channel = grpc.insecure_channel(hostport)
	stub = prediction_service_pb2_grpc.PredictionServiceStub(channel)
	model_request = predict_pb2.PredictRequest()
